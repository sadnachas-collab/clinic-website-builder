/* ================================================================
   Загрузка данных сайта через локальный API Timeweb.
   Подключается после /clinic-api-client.js, до закрывающего </body>.
   ================================================================ */
(function () {
  "use strict";

  if (!window.supabase || !window.supabase.createClient) {
    console.error("[site] локальный API-клиент не загрузился");
    return;
  }
  ...
  function sanitizeRichHtml(value) {
    ...
    var allowedColors = { "#2A2522": true, "#746258": true, "#b91c1c": true };
      ...
      Array.prototype.slice.call(parent.childNodes).forEach(function (node) {
        ...
        Array.prototype.slice.call(el.attributes).forEach(function (attr) {
          ...
          if (el.tagName === "A" && name === "href") {
            if (!/^(https?:|mailto:|tel:)/i.test(attr.value)) el.removeAttribute(attr.name);
            return;
          }
          if (el.tagName === "SPAN" && name === "style") {
            var match = attr.value.match(/(?:^|;)\s*color\s*:\s*([^;]+)/i);
            var color = match ? match[1].trim() : "";
            if (allowedColors[color]) el.setAttribute("style", "color: " + color);
  ...
  function splitEduBio(text) {
    var sep = "\n\n---\n\n";
    var raw = String(text || "");
    var idx = raw.indexOf(sep);
    if (idx === -1) return { edu: "", bio: raw.replace(/\s+---\s+/g, " ") };
    return { edu: raw.slice(0, idx), bio: raw.slice(idx + sep.length) };
  }
  function splitReviewText(text) {
    var raw = String(text == null ? "" : text);
    var match = raw.match(/^\[([^\]]+)\]\s*([\s\S]*)$/);
    if (!match) return { service: "", text: raw };
    return { service: match[1].trim(), text: match[2] || "" };
  }
    ...
    if (mainTexts && mainTexts.hero) {
      ...
      if (titleEl && mainTexts.hero.title) {
        ...
        if (lines.length >= 3) {
          titleEl.innerHTML =
            esc(lines[0]) +
            '<br><span class="text-[#746258] italic font-light tracking-normal">' +
            esc(lines[1]) +
            "</span><br>" +
            esc(lines.slice(2).join(" "));
    ...
    host.innerHTML = rows
      .map(function (p) {
        ...
        return (
          ...
          escJs(bookLabel) +
          '\')" class="relative flex-shrink-0 w-[85vw] sm:w-[350px] lg:w-[380px] xl:w-[400px] aspect-[4/5] sm:aspect-[4/3] lg:aspect-[4/3] rounded-[2rem] overflow-hidden group snap-center cursor-pointer shadow-sm hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] active:scale-[0.98] transition-all duration-500">' +
          '<img src="' +
          escAttr(img) +
          '" alt="' +
          titleEsc +
          ...
          '<h3 class="text-white font-serif text-2xl lg:text-3xl mb-2 tracking-tight">' +
          titleEsc +
          "</h3>" +
          (descEsc
            ? '<p class="text-white/90 text-sm lg:text-base font-light leading-relaxed mb-6">' +
              descEsc +
              "</p>"
            : "") +
  ...
  function renderCategoriesAndProcedures(cats, procs) {
    ...
    cats.forEach(function (c) {
      catMap[c.id] = {
        ...
        img:
          c.image_url ||
          "https://images.unsplash.com/photo-1616394584738-fc6e612e71b9?auto=format&fit=crop&q=80&w=1600",
        procedures: [],
      };
    });
    (procs || []).forEach(function (p) {
      ...
      procMap[p.id] = {
        ...
        desc: p.description || "",
        howItWorks: howRaw.length
          ? howRaw
          : [{ q: "Описание", a: p.description || "Подробности уточняйте на консультации." }],
        problems: probRaw.length
          ? probRaw
          : [{ q: "Подробности", a: "Уточните у врача на консультации." }],
        prices: pricesRaw.length
          ? pricesRaw
          : p.price
            ? [{ name: p.name, vol: "—", time: "—", cost: p.price }]
            : [],
      };
  ...
  function renderSpecialists(rows) {
    ...
    inner.innerHTML = rows
      .map(function (d) {
        ...
        var details = splitEduBio(d.description);
        var bio = esc(details.bio || "");
        var img =
          d.photo_url ||
        ...
        var args =
          ...
          "'";
        return (
          '<div class="relative flex-shrink-0 w-[85vw] sm:w-[320px] lg:w-[280px] flex flex-col items-center group h-full snap-center bg-[#FDFBF9] rounded-[2.5rem] p-6 shadow-sm hover:shadow-[0_15px_35px_rgb(0,0,0,0.06)] border border-[#D0C0B1]/20 transition-all duration-500">' +
          ...
          '<img src="' +
          escAttr(img) +
          '" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 ease-out" alt="' +
          name +
          '">' +
          "</div>" +
          '<h3 class="font-serif text-2xl text-[#2A2522] mb-1 text-center">' +
          name +
          "</h3>" +
          '<p class="text-[10px] text-[#746258] uppercase tracking-[0.2em] font-bold mb-5 text-center">' +
          role +
          "</p>" +
          ...
          '<p class="text-[11px] text-gray-500 leading-relaxed font-light line-clamp-3">' +
          bioLine +
          "</p>" +
          "</div>" +
          '<button onclick="openDoctor(' +
          args +
          ')" class="w-full bg-[#746258] text-white px-5 py-3 rounded-full text-xs uppercase tracking-widest font-bold hover:bg-[#5A4C44] active:scale-95 transition-all shadow-md">Профиль</button>' +
          "</div>"
        );
      })
      .join("");
  ...
  function renderReviews(rows) {
    ...
    host.innerHTML = rows
      .map(function (r) {
        var parsed = splitReviewText(r.text);
        var rating = Math.max(0, Math.min(5, parseInt(r.rating, 10) || 5));
        var stars = "";
        for (var i = 0; i < rating; i++)
          stars += '<i data-lucide="star" class="w-4 h-4 fill-current"></i>';
        return (
          '<div class="bg-white p-8 rounded-[2.5rem] border border-[#D0C0B1]/20 relative group hover:shadow-[0_15px_40px_rgb(0,0,0,0.04)] transition-all duration-500 flex flex-col">' +
          '<div class="flex gap-1 mb-6 text-[#D0C0B1] group-hover:text-[#746258] transition-colors duration-500">' +
          stars +
          "</div>" +
          '<p class="text-gray-600 font-light text-[15px] leading-relaxed mb-8 italic relative z-10 flex-grow">"' +
          esc(parsed.text || "") +
          '"</p>' +
          '<div class="pt-6 border-t border-[#D0C0B1]/30 flex justify-between items-end mt-auto">' +
          '<div><p class="font-medium text-[#2A2522]">' +
          esc(r.author_name || "") +
          "</p>" +
          (parsed.service
            ? '<p class="text-[11px] text-[#746258] mt-1.5 uppercase tracking-widest font-semibold">' +
              esc(parsed.service) +
              "</p>"
            : "") +
          (r.review_date
            ? '<p class="text-[11px] text-gray-400 mt-1.5 uppercase tracking-widest font-semibold">' +
              esc(r.review_date) +
              "</p>"
            : "") +
          "</div></div>" +
    ...
    host.innerHTML = rows
      .map(function (f, idx) {
        ...
        return (
          ...
          '<span class="font-serif text-xl lg:text-2xl text-[#2A2522] pr-4 group-hover:text-[#746258] transition-colors duration-300">' +
          esc(f.question || "") +
          "</span>" +
          ...
          '<div class="text-gray-500 font-light leading-relaxed pr-8 lg:pr-12 text-[15px] mt-4">' +
          sanitizeRichHtml(f.answer || "") +
          "</div>" +
          "</div></div>"
        );
      })
  ...
  function renderDocs(rows) {
    ...
    if (!rows || !rows.length) {
      host.innerHTML =
        '<div class="bg-[#FDFBF9] rounded-[2.5rem] p-8 border border-[#D0C0B1]/20 text-center"><p class="text-sm text-gray-400 italic">Документы появятся здесь после загрузки в админ-панели.</p></div>';
      return;
    }
    host.innerHTML = rows
      .map(function (d) {
        ...
        var category = d.category
          ? '<p class="text-[10px] uppercase tracking-widest text-gray-400 font-semibold mb-1">' +
            esc(d.category) +
            "</p>"
          : "";
        return (
          '<a href="' +
          escAttr(url) +
          '" target="_blank" rel="noopener noreferrer" class="flex items-center p-5 bg-[#FDFBF9] rounded-2xl border border-[#D0C0B1]/30 hover:border-[#746258] hover:shadow-md transition-all group">' +
          ...
          category +
          '<p class="text-[15px] text-[#2A2522] font-medium truncate">' +
          title +
          "</p></div>" +
    ...
    var list = files
      .map(function (f) {
        ...
        return (
          '<a href="' +
          escAttr(url) +
          '" target="_blank" rel="noopener noreferrer" download class="flex items-center justify-between p-5 bg-[#FDFBF9] rounded-2xl border border-[#D0C0B1]/30 hover:border-[#746258] hover:shadow-md transition-all group">' +
  ...
  function renderConsumer(cats, docs, auth) {
    ...
    try {
      localStorage.setItem("dbConsumer", JSON.stringify(data));
    } catch (_) {}
  ...
  function renderContactsAndFooter(mainTexts, footer) {
    ...
    if (footerEl) {
      ...
      if (cols[0]) {
        ...
        if (ul) {
          ...
          if (items[2] && schedule) {
            var sp = items[2].querySelector("span");
            if (sp) sp.textContent = schedule;
          }
        }
      }
    ...
    // Соцссылки в шапке и плавающем виджете.
    var social = { vk: contacts.vk, tg: contacts.tg, max: contacts.max || contacts.wa };
    var allLinks = Array.prototype.slice.call(document.querySelectorAll("a"));
    function activateSocialLink(link, url) {
      if (!link || !url) return;
      link.href = url;
      link.target = "_blank";
    ...
    allLinks.forEach(function (link) {
      ...
      var hasTelegramIcon = !!link.querySelector('[data-lucide="send"]');
      if (text === "vk") activateSocialLink(link, social.vk);
      if (hasTelegramIcon) activateSocialLink(link, social.tg);
      if (text === "max") activateSocialLink(link, social.max);
    });
    refreshIcons();
  }
  ...
  function renderAboutDoctor(v) {
    ...
    if (photoEl) {
      ...
      if (wantVideoPreview && !isVideoNode) {
        ...
        img.src =
          v.photo ||
          "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 4 5'/>";
  ...
  window.openAboutDoctorVideo = function () {
    ...
    if (ytMatch) {
      embed =
        ...
        ytMatch[1] +
        '?autoplay=1&rel=0" allow="autoplay; encrypted-media; fullscreen" allowfullscreen class="w-full h-full rounded-2xl"></iframe>';
    } else if (vimeoMatch) {
      embed =
        '<iframe src="https://player.vimeo.com/video/' +
        vimeoMatch[1] +
        '?autoplay=1" allow="autoplay; fullscreen" allowfullscreen class="w-full h-full rounded-2xl"></iframe>';
    } else {
      embed =
        '<video src="' +
        escAttr(url) +
        '" controls autoplay playsinline class="w-full h-full bg-black rounded-2xl"></video>';
    }
  ...
  async function loadAll() {
    ...
    var pcMap = {};
    (pc.data || []).forEach(function (row) {
      pcMap[row.key] = row.value || {};
    });
    ...
    try {
      renderHero(interior.data, mainTexts);
    } catch (e) {
      console.error("[site] hero", e);
    }
    try {
      renderPromos(promos.data);
    } catch (e) {
      console.error("[site] promos", e);
    }
    try {
      renderCategoriesAndProcedures(cats.data, procs.data);
    } catch (e) {
      console.error("[site] categories", e);
    }
    try {
      renderSpecialists(specs.data);
    } catch (e) {
      console.error("[site] specialists", e);
    }
    try {
      renderReviews(revs.data);
    } catch (e) {
      console.error("[site] reviews", e);
    }
    try {
      renderFaq(faqs.data);
    } catch (e) {
      console.error("[site] faq", e);
    }
    try {
    ...
    try {
      ...
      console.error("[site] prices", e);
    }
    try {
      renderConsumer(ccats.data, cdocs.data, auth.data);
    } catch (e) {
      console.error("[site] consumer", e);
    }
    try {
      renderContactsAndFooter(mainTexts, footerCfg);
    } catch (e) {
      console.error("[site] footer", e);
    }
    try {
      renderReviewLinks(mainTexts);
    } catch (e) {
      console.error("[site] revlinks", e);
    }
    try {
      renderAboutDoctor(aboutDoctor);
    } catch (e) {
      console.error("[site] about-doctor", e);
    }
  }
